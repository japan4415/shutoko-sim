...
現在は企画・設計段階です。アプリケーション、探索エンジン、デプロイ環境は未実装で、利用できる公開サービスや起動コマンドはまだありません。
...
最初に企画と要件を読み、実装時にはシステム設計、探索、インターフェースの順に参照してください。設計は原案を具体化した提案であり、実測値や実装済み仕様ではありません。外部仕様の参照確認日は 2026-09-10 です。
