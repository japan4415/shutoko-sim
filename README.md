...
- **検証済み課金区間**: C1 外回り 4 区間・内回り 5 区の合計 9 ペア（`bp:c1-outer:kandabashi-takaracho` ほか。公式路線図および公式料金表・改定発表に基づき、いずれも普通車 ETC 300 円の実料金・有効期間を登録済み。出典情報は `manifest.json` に記録、一覧は `docs/data-pipeline.md` 参照）。
- **未検証事項**:
