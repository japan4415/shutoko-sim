...
- **最初の出口（First Exit）検証**: 本線基準点（`anchorOsmNodeId`）から首都高速本線エッジを経由して最初に到達可能な出口エッジのみが検証済みペアとして採用可能。後続の出口（例: 神田橋入口・宝町出口に対してさらに下流の新富町出口）は `FIRST_EXIT_MISMATCH` として拒否される。探索アルゴリズムは walk 意味論（ノード再訪許容）に基づいて `(node, suffix)` 状態キーによる証明可能に健全な枝刈りを行い、過去の訪問ノード履歴に起因する行き止まりや合法な最短出口の見落としを数学的に排除する（なお、課金ペア自体の直接経路は別途単純路制約が厳格に課される）。
