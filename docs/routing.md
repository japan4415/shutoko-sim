...
出発地点 `s`、時間下限 `L`、上限 `U` に対して、座標検索では検証済み課金ペアを選ぶ。`entryRampId` と `exitRampId` の双方を指定する検索では、課金ペアseedの有無にかかわらず、`routingCapability=routable` の一般入口・一般出口間に「入口→アプローチ→本線閉路→イグレス→出口」を構成する。境界JCT、閉鎖、unsupported、`structural_no_loop`、kind不一致は探索端点にしない。出口から一般道で `s` へ戻ることは今回の企画提案で、首都高上の一周とは区別する。
...
### 東京駅の既知の時間境界と意図的 deviation

誤帰属 binding と曖昧な課金ペアを安全のため除外し、座標検索に使う verified pair を2件へ縮退させた結果、東京駅（35.6812, 139.7671）の現行全線グラフでは15〜60分に神田橋入口→宝町出口の1件だけが成立する。実測の `minPlanSeconds` は1,743秒（約29.05分）であり、上限29分では `TIME_WINDOW`、上限30分では成立する。一方、`T_base` が30分未満のため、旧データで成立した30〜60分条件は `no_candidates/TIME_WINDOW` となり、上限を240分へ広げても別候補は生じない。これは候補数より施設帰属と課金端点の正確性を優先した意図的 deviation であり、30〜60分の成立を保証しない。
...
2. **探索モードの選択**:
   ...
   - 座標入力時は、選定された近傍入口アクセス地点（`access_node_indices`）に接続する課金ペアのみを**`max_pairs` での打ち切り前に抽出**する。`max_access_entries` が有限なら近距離でも対象ペアが脱落して `NO_CONNECTION` になり得るため、UIは距離cap超過と同一視しない。
