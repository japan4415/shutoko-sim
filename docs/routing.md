...
2. **探索モードの選択**:
   ...
   - 座標・ノード検索（および片方のみ指定）は、近傍の入口アクセス地点を出発地からの距離順にソートした入口 tier を評価する最近接入口優先探索を行う。各 tier 内で検証済み課金ペアがあれば優先評価し、なければ構造的に利用可能な一般出口への動的 OD を探索する。
   - 完全評価された最近接 tier に合法周回が存在する場合は、その tier が `TIME_WINDOW` / `NO_HANDOFF` 診断を担い、遠方の入口へフォールスルーせず確定する。構造的に周回が存在しない（structurally dead）最近接 tier のみ後続 tier へフォールスルーする。
   - 料金根拠のない動的 OD 候補を含む場合、候補集合全体の順位付けは `shutoko_time`（首都高走行時間順）に切り替える。
