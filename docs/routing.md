...
現行 C1 の `anchorOsmNodeId` は、開始と終了が同じノードである `sameNodeLoopAnchor` の特殊例として扱う。既存の anchor、First Exit、料金レコード、検証状態は変えない。
...
| 型 | 主なフィールド | 用途 |
| --- | --- | --- |
| `sameNodeLoopAnchor` | `nodeId`, `routeId`, `direction`, `arcPolicy` | 現行 C1。開始と終了を同じノードとして表す。 |
| `directedJunctionLoopAnchor` | `mergeNodeId`, `branchNodeId`, `mergeTerminalEdgeId`, `branchInitialEdgeId`, `routeId`, `direction`, `arcPolicy`, `excludedShortConnector` | 放射線 → 環状線 → 放射線。M と B を directed JCT pair として表す。 |

`anchorNodeId`、`entryToAnchorEdgeIds`、`anchorToExitEdgeIds` は legacy C1 variant 専用である。radial variant に M を `anchorNodeId` として詰め込み、B までの経路を偽の post-anchor path へ押し込まない。正本となるのは M/B pair と `resolvedRouteSegments` である。

### route と direction の所属を Graph 上の索引で証明する

現行 Edge には route membership と走行方向がない。edge kind だけで First Exit を求めると、B 付近の C1 出口や別 arm への近道を先に拾う。そこで **graph schema 4 の top-level `routeMemberships[]` に `RouteMembershipIndex` を持たせる**方式に決める。Edge ごとに同じメタデータを複製するより、OSM relation の順序・方向・由来を一つの索引として検索、検証、manifest へ渡せるためである。
...
生成時、OSM relation の ordered member と way のノード順を、生成済み有向 Edge ID へ写像する。route plan の entry、loop、return は `membershipId` で index を参照し、各 resolved segment は参照先 `orderedEdgeIds` の連続部分列でなければならない。境界の terminal/initial Edge も seed の宣言と一致させる。`sourceRelationId`、snapshot hash、resolved exact Edge 列、segment hash は build 時に index と graph へ出力し、実行時検査の正本とする。名前や最接近ノードだけで所属を補わない。

次の異常系テストを必須とする。

- C1 内回りを要求しているのに outer の member Edge を使う。
- 一ノ橋 JCT で別 arm へ切り替える近道を使う。
- route relation の member に含まれない way を使う。
- 同名または近接した別 JCT の Edge を使う。
- excluded short connector を mandatory lap として選ぶ。
- relation の順序と逆順にたどる。
...
この2件は本 PR で wire-level schema と route shape を確定した課金ペア設計である。実装 issue では、schema 適合の diagnostic fixture と C1 非回帰テストを作る。天現寺 exact directed binding が解決し、route/direction、First Exit、全端点がすべて通ったときだけ `Graph.billingPairs` の `radialReturn` として昇格する。解決前の plan を public candidate として出さない。

目黒入口 → 目黒出口の現行 dynamic OD は別分類にする。entry Edge は `e:w207535708:0:f`、exit Edge は `e:w207535709:0:f` で、routing topology 上は到達可能である。しかし物理的には天現寺 Exit が先であり、exact binding がなければ目黒を「1区間先」にできない。したがって `topology_only` とし、「1区間先」「最低料金」、`time_per_yen` の対象から外す。`routingCapability=routable` は道路を追跡できることを示すが、商品 eligibility の証拠ではない。

天現寺・荏原・戸越入口については、現行 support data で entry / exit の exact directed binding が未解決である。候補 way や施設名を nearest node へ割り当てて補わない。天現寺 binding issue は multi-way ramp の全 way 候補、ground ↔ mainline の接続、ramp ID の逆引き、topology classification を同じ evidence model で扱う。

### C1 8ペアは legacy adapter で変更しない

8件はいずれも `pairKind` を持たない現行 v1 形式として読む。schema 2 の seed でも配列要素の意味と値は変えず、生成 graph の schema 4 で `pairKind=legacyRing` として明示する。`sameNodeLoopAnchor`、entry/exit、First Exit の判定、300円→300円の legacy price records、verified 2件・unverified 6件を保つ。
...
時間計算の式は変えない。首都高時間は、完成した順序付き Edge 列の各出現について `durationSeconds` を合計した値である。2号往路、C1 mandatory lap、2号復路をそれぞれ1回ずつ数える。dedup した Edge 集合から所要時間を組み立てない。

Candidate v2 は、高速道路の区間と一般道の概算区間を別配列にする。

- `edgeRouteLegs`: `edgeIds` の index を持ち、次の4 roleだけを使う。`entry_approach`、`mandatory_lap`、`return_corridor`、`exit_approach`。
- `estimatedLegs`: `surface_access` と `surface_return` を持ち、`estimated=true`、Edge index と Edge 几何を持たない。

`startEdgeIndex` は包含、`endEdgeIndexExclusive` は除外とする。`edgeRouteLegs` を Candidate 順に並べると、`[0, edgeIds.length)` を重複も欠落もなく覆うことを契約テストにする。同じ Edge ID が複数 index に現れるのは許すが、index 範囲が重なるのは許さない。`post_lap_transfer` という別 role は採用せず、seed、generated graph、Candidate、UI で `return_corridor` だけを使う。

`geometry` は首都高の Edge に対応する線分を連結したもので、Web が推定する一般道区間は含めない。地図と詳細画面には `entry_approach → mandatory_lap → return_corridor → exit_approach` を番号とテキストで示す。色だけで経路の順序を示さない。画面上の surface access / return は Edge 付き経路に含めず、推定時間として別に表示する。
...
新 routing contract では `eligibilityStatus`、`amountYen`、`billingDistanceMeters`、`tariffStatus` を正本にする。`chargedSectionCount=1` と `ONE_SECTION_TOLL` は、旧 C1 output だけを受け持つ legacy adapter に限定する。radial output には `chargedSectionCount` を出さず、`ONE_SECTION_TOLL` も生成しない。「1区間先関係」と「料金制度上の1区間」を同一表示にしない。

Issue #41 で公式 billing distance と版管理済み料金規則を確定するまでは、次の規律を適用する。
...
- 動的 OD は `topology_only` とし、商品 ranking cohort に入れない。
...
graph schema 4 を builder だけが先に出力する段階は作らない。現行 engine は unknown field を拒否するため、新 schema の reader と release wiring が整うまで、公開 builder の既定出力は現行 schema のまま維持する。

| Issue | 実装範囲 | 主な受け入れ条件 | 依存 |
| ---: | --- | --- | --- |
| 1 | seed schema v2 と diagnostic pair 型 | 既存 C1 8要素を無改変で読む。`pairKind` なしは legacy、新 radial は明示 `pairKind=radialReturn` と version 必須。未知 kind/version を拒否。wire-level JSON fixture を追加し、C1 8件の ID・意味・価格・状態・anchor が一致。 | なし |
| 2 | `RouteMembershipIndex` と OSM relation provenance | route/direction の順序付き Edge 索引と hash を生成。逆方向・同名 JCT・非所属 way・別 arm 近道・short connector の異常系テストが通る。 | 1 |
| 3 | directed mandatory lap と return-corridor First Exit | synthetic radial fixture で M→B 長弧、return corridor、first general Exit を分解。C1 legacy の完全一致を維持。segment 内反復は禁止し、route plan が宣言する segment 間反復は許可する。 | 1, 2 |
| 4 | graph schema 4 reader と consumer 契約 | core、WASM 型、Web Worker が schema 2/3/4 を読み、legacy / radial の union を判別。未知 kind/version、部分 data、route leg の重複・欠落を拒否。公開 release はまだ変更しない。 | 1, 3 |
| 5 | graph schema 4 の atomic release activation | builder の既定 output 切替、core reader、WASM contract、Web pipeline、Workers artifact allowlist、新しい versioned release ID、manifest hash を同時に整合。旧 release は rollback 用に残す。 | 2, 3, 4 |
| 6 | 天現寺 exact directed binding | multi-way ramp corpus、ground ↔ mainline topology、ramp ID inverse-map、公式施設順を同じ support evidence として設計・実装。解決できなければ根拠付き unsupported のままにする。 | なし |
| 7 | 2号 inner / outer radial pair 統合 | schema 適合 fixture と C1 non-regression が通る。exact binding 未完なら diagnostic plan のみ。完了時だけ Graph radial pair と public eligibility へ昇格し、#41 まで tariff は未算出。 | 3, 5, 6 |
| 8 | Candidate route legs と product / tariff 状態 | synthetic radial fixture で4 highway legs が Edge 列を重複なく被覆し、surface legs は推定値。eligibility / tariff / topology_only を分離し、radial に `chargedSectionCount` や `ONE_SECTION_TOLL` を出さない。 | 4 |
| 9 | Web の順序表示 | entry / lap / return / exit を番号・線種・テキストで提示し、surface 概算と highway 経路を混同しない。unpriced と topology_only に「1区間料金」を表示しない。C1 UI regression を維持。 | 5, 8 |
| 10 | split Maps URL 生成 | 3 waypoint / 2,048文字制限、leg ごとの手動継続、URL builder unit / E2E を実装。道路・向きを強制できないため、実機 gate 通過まで public handoff は disabled。 | 8 |
| 11 | Maps 実機検証と release gate | Android / iOS × Web / app の必要 matrix を manifest に記録。失敗・期限切れで radial public departure を無効にし、C1 への副作用がないことを確認。device 未接続でも code issue 10 は完了可能。 | 10 |
| 12 | Issue #41 後の tariff 統合 | 公式 billing distance、車種、税率、単価、最低・上限、丸め、effective interval を版管理し、C1 8件と2号代表 pair を再検証。OSM distance fallback と radial の `time_per_yen` 無効状態を維持しない。 | #41, 7, 8 |
