...
2. **対象課金ペアの選定と事前絞り込み（Pre-filtering before max_pairs）**:
   - リクエストに明示的なランプ指定（`entryRampId` / `exitRampId`）がある場合、該当するランプ ID またはエッジ ID を持つ課金ペアのみに絞り込む。
...
3. **本線閉路探索とマイクロループ排除**:
   - 各ペアの基準点（アンカー）から同じ基準点へ戻る有向閉路を訪問履歴付き探索で列挙する。空閉路は禁止。
...
6. **料金算出とスキーマ分離**:
   ...
   - **公式普通車 ETC 料金（`amount_yen`）**:
     ...
     - 料金距離 > 4.3km: `(距離km × 29.52 + 150) × 1.10`（10円単位四捨五入、上限 1,950 円）
...
本線閉路（loop）の探索は訪問履歴を持つ幅制限探索とし、残り帰還時間の下界と退出・一般道帰路の時間を用いて、上限内で帰れない状態を枝刈りする。訪問履歴の異なる状態を同一ノードの最短時間だけで統合しない。

既定値は深さごとの保持状態200、全ペア合計の展開状態100,000、閉路の最大エッジ数5,000（広域周回対応）、最小ループ長5,000mとする。いずれかの上限到達は `truncated` とし、検証済みの候補だけ返す。UI は探索開始10秒で Web Worker を終了でき、その場合は候補を返さず再試行を案内する。同じ入力・成果物版・探索上限では同じ展開順と結果にする。
