...
既定値は深さごとの保持状態200、全ペア合計の展開状態100,000、閉路の最大エッジ数2,000とする。一般道エッジを除外したことにより、実データは 8 課金ペア・1,719 エッジ（shutoko 1,686 / entry 16 / exit 17）まで削減された。アクセス時間を概算に切り替えたことで本線閉路探索だけを対象とし、既定上限100,000の範囲内で安定して動作する。いずれかの上限到達は `truncated` とし、検証済みの候補だけ返す。UI は探索開始10秒で Web Worker を終了でき、その場合は候補を返さず再試行を案内する。同じ入力・成果物版・探索上限では同じ展開順と結果にする。
